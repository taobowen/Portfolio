...
function renderMarkdownPages(docDir, distDir) {
  const pagesDir = path.join(distDir, 'pages');
  const coversDir = path.join(distDir, 'covers');
  const metaFile = path.join(distDir, 'rawList.json');
  const metadataList = [];
...
`;
  ...
  fs.writeFileSync(path.join(distDir, 'pageMap.js'), pageMapContent, 'utf-8');
}
...
function generateSite(configPath) {
  ...
  // Copy template
  fse.copySync(templateDir, distDir);
  ...
  if (config.favicon && fs.existsSync(config.favicon)) {
    const destFavicon = path.join(distDir, path.basename(config.favicon));
    fs.copyFileSync(config.favicon, destFavicon);
  }

  // Render project pages
  if (fs.existsSync(docDir)) {
    renderMarkdownPages(docDir, distDir);
  }
